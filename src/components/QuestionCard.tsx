...
import { PlayableQuestion, SubjectGroup } from '../types';
...
interface QuestionCardProps {
  ...
  cutStart?: string;
  cutEnd?: string;
  tocItems?: SubjectGroup[];
  activeSubject?: string | null;
  onSelectTocSubject?: (subject: string) => void;
  onUpdateParams?: (
    newShuffleQ: boolean,
    newShuffleO: boolean,
    newAutoAdv: boolean,
    ...
    <div id="question-card-wrapper" className="space-y-4 max-w-2xl mx-auto">
      ...
      <div className="bg-white dark:bg-[#161B22] border border-slate-200 dark:border-slate-800 rounded-2xl px-4 py-3 shadow-sm flex items-center justify-between gap-4 font-sans relative">
        ...
        <div className="absolute left-1/2 -translate-x-1/2 hidden md:flex items-center justify-center pointer-events-none select-none">
          <span className="text-xs font-sans font-black uppercase tracking-[0.2em] text-slate-400 dark:text-slate-500">
            SIMED
          </span>
        </div>
                ...
                <>
                  ...
                  >
                    ...
                    <div className="space-y-1">
                      ...
                      {tocItems.length > 0 && onSelectTocSubject && (
                        <button
                          type="button"
                          onClick={() => {
      ...
      <AnimatePresence>
        {showToc && tocItems.length > 0 && onSelectTocSubject && (
          <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <motion.aside
              ...
              className="relative h-full w-full max-w-sm bg-white dark:bg-[#0B0F16] border-l border-slate-200 dark:border-slate-800 shadow-2xl p-5 overflow-y-auto font-sans"
            >
              <div className="flex items-start justify-between gap-3 border-b border-slate-200 dark:border-slate-800 pb-4 mb-4">
                <div>
                  <p className="text-[10px] uppercase font-black tracking-widest text-slate-400">რეზიდენტურის ბაზა</p>
                  <h2 className="text-lg font-black text-slate-900 dark:text-white">სარჩევი</h2>
                </div>
                <button
                  type="button"
                  onClick={() => setShowToc(false)}
                  className="p-2 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-500 hover:text-slate-900 dark:hover:text-white bg-slate-50 dark:bg-slate-900 focus:outline-none"
                  title="დახურვა"
                >
                  ×
                </button>
              ...
              <div className="space-y-2">
                <button
                  type="button"
                  onClick={() => {
                    onSelectTocSubject('__all__');
                    setShowToc(false);
                  }}
                  className={`w-full px-3.5 py-3 rounded-2xl border text-left transition-all focus:outline-none ${
                    activeSubject === null
                      ? 'bg-slate-950 dark:bg-white text-white dark:text-slate-950 border-slate-950 dark:border-white shadow-sm'
                      : 'bg-slate-50 dark:bg-[#111827] text-slate-800 dark:text-slate-200 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600'
                  }`}
                >
                  <span className="flex items-center justify-between gap-3">
                    <span className="text-sm font-extrabold">ყველა მიმართულება</span>
                    <span className={`text-[11px] font-bold ${activeSubject === null ? 'text-slate-300 dark:text-slate-600' : 'text-slate-400 dark:text-slate-500'}`}>
                      {tocItems.reduce((sum, item) => sum + item.count, 0)} გვ.
                    </span>
                  </span>
                </button>
                ...
                {tocItems.map((item) => {
                  ...
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        onSelectTocSubject(item.subject);
                        setShowToc(false);
                      }}
                      className={`w-full px-3.5 py-3 rounded-2xl border text-left transition-all focus:outline-none ${
                        isActive
                    ...
                    >
                      <span className="flex items-center justify-between gap-3">
                        <span className="text-sm font-extrabold">{item.subject}</span>
                        <span className={`text-[11px] font-bold ${isActive ? 'text-slate-300 dark:text-slate-600' : 'text-slate-400 dark:text-slate-500'}`}>
                          {item.count} გვ.
                        </span>
                      </span>
                    </button>
