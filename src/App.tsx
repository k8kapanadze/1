...
import { attachSubjects, buildSubjectGroups, parseQuestionFile, prepareSessionQuestions, serializeToCustomFormat } from './utils';
import residencyBankText from './data/residencyBank.txt?raw';
...
export default function App() {
  ...
  // Chosen mode: practice = Standalone Practice, simulator = Exam Simulator, residency = bundled residency bank
  const [chosenMode, setChosenMode] = useState<'practice' | 'simulator' | 'residency' | null>(null);
  const [activeResidencySubject, setActiveResidencySubject] = useState<string | null>(null);
  ...
  const residencyQuestions = useMemo(() => {
    return attachSubjects(parseQuestionFile(residencyBankText, 'რეზიდენტურის ბაზა'));
  }, []);

  const residencySubjectGroups = useMemo(() => {
    return buildSubjectGroups(residencyQuestions);
  }, [residencyQuestions]);
  ...
  const handleStartResidencyBank = (subject?: string) => {
    const selectedSubject = subject === '__all__' ? undefined : subject;
    const pool = selectedSubject
      ? residencyQuestions.filter((question) => question.subject === selectedSubject)
      : residencyQuestions;

    if (pool.length === 0) return;

    const playable = prepareSessionQuestions(pool, false, false);
    setChosenMode('residency');
    setActiveResidencySubject(selectedSubject || null);
    setSession({
      type: 'residency',
      sourceFiles: ['რეზიდენტურის ბაზა'],
      originalQuestions: pool,
      questions: playable,
      currentIndex: 0,
      answers: {},
    });
    ...
    <div className="min-h-screen bg-white dark:bg-[#0A0C10] font-sans transition-colors duration-200">
      ...
      {!session && (
        <div className="max-w-4xl mx-auto px-4 pt-6">
          <header className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between bg-[#151B24] border border-slate-800 rounded-2xl px-6 py-4 shadow-[0_18px_45px_rgba(2,6,23,0.28)] gap-4 transition-all">
            <div className="flex items-center gap-3 cursor-pointer" onClick={() => setSession(null)}>
              <div className="w-12 h-12 bg-[#221B2A] rounded-2xl flex items-center justify-center shadow-[0_0_28px_rgba(190,18,60,0.35)] text-white select-none border border-rose-500/10">
                <Stethoscope className="w-5 h-5 text-white" />
              </div>
              <div>
                <h1 className="text-xl font-black tracking-tight text-white flex items-center gap-2">
                  SIMED
                  <span className="text-[#4EA4FF] font-extrabold text-[9px] uppercase tracking-wider px-2.5 py-1 border border-[#2563EB]/40 rounded-full bg-[#0F2A4D]/55">
                    Status In Medicina
                  </span>
                </h1>
              </div>
        ...
        {session ? (
          <div id="practicing-session-view" className="space-y-6">
            ...
            {session.currentIndex < session.questions.length ? (
              <QuestionCard
                ...
                tocItems={chosenMode === 'residency' ? residencySubjectGroups : []}
                activeSubject={activeResidencySubject}
                onSelectTocSubject={chosenMode === 'residency' ? handleStartResidencyBank : undefined}
              />
            ) : (
              ...
              <ResultsView
                ...
                onRestart={() => {
                  setSession(null);
                  if (chosenMode === 'residency') {
                    setChosenMode(null);
                    setActiveResidencySubject(null);
                  }
                }}
          ...
          <div id="welcome-mode-selection" className="space-y-8 animate-fade-in text-center py-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-5xl mx-auto pt-4">
              ...
              {/* Card 3: Residency Bank */}
              <div
                onClick={() => handleStartResidencyBank()}
                className="group relative bg-[#0B0F16] border border-slate-800 rounded-3xl p-6 md:p-8 text-left shadow-sm hover:shadow-xl hover:shadow-black/20 hover:border-slate-500 transition-all duration-300 cursor-pointer flex flex-col justify-between overflow-hidden"
              >
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_15%,rgba(148,163,184,0.14),transparent_34%)] pointer-events-none" />
                <div className="relative">
                  <div className="flex justify-between items-start mb-6">
                    <span className="text-[10px] font-black uppercase text-slate-300 bg-white/5 px-2.5 py-1 rounded-md border border-white/10">
                      რეჟიმი 03
                    </span>
                    <div className="p-3 bg-white/5 text-slate-200 rounded-2xl border border-white/10 group-hover:scale-110 transition-transform duration-300 shadow-[0_0_26px_rgba(15,23,42,0.45)]">
                      <BookOpen className="h-6 w-6" />
                    </div>
                  </div>
                  <h3 className="text-lg md:text-xl font-bold text-white mb-2 group-hover:text-slate-200 transition-colors">
                    რეზიდენტურის ბაზა
                  </h3>
                  <p className="text-sm text-slate-400 leading-relaxed mb-6 font-medium">
                    ჩაშენებული თერაპიის კითხვები, მუქი ფოკუს-რეჟიმი და სარჩევი საგნების მიხედვით.
                  </p>
                </div>
                <div className="relative text-xs font-bold text-slate-200 flex items-center gap-1.5 transition-all group-hover:translate-x-1">
                  <span>დაწყება</span>
                  <ArrowRight className="h-4 w-4" />
                </div>
              </div>
